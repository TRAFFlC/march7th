...
if _gpt_sovits_dir:
    GPT_SOVITS_DIR = Path(_gpt_sovits_dir)
else:
    GPT_SOVITS_DIR = Path(BASE_DIR.parent / "GPT-SoVITS-v2pro-20250604")

_tts_gpt_weight = PATH_CONFIG.get("tts_gpt_weight", "")
TTS_GPT_WEIGHT = _tts_gpt_weight if _tts_gpt_weight else ""

_tts_sovits_weight = PATH_CONFIG.get("tts_sovits_weight", "")
TTS_SOVITS_WEIGHT = _tts_sovits_weight if _tts_sovits_weight else ""
...
REF_AUDIO_PATH = _ref_audio_path if _ref_audio_path else ""
